...
export class SolarSystemManager {
        ...
        try {
            ...
            if (this.universe) {
                ...
                if (systemData) {
                    ...
                    console.log('🌌 System not found in universe data, trying API then fallback');
                    ...
                    try {
                        ...
                        console.log('🌌 API not available (standalone mode) - generating fallback star system');
                        this.starSystem = this.generateFallbackStarSystem(sector);
                    }
                }
            } else {
                console.log('🌌 No universe data available, trying API then fallback');
    ...
    generateFallbackStarSystem(sector) {
        console.log('Generating fallback star system for sector:', sector);
        
        // Create a simple hash from sector name for consistent generation
        const hash = this.simpleHash(sector || 'A1');
        const rand = this.seededRandom(hash);
        
        // Generate star properties
        const starTypes = ['G', 'K', 'M', 'A', 'F'];
        const starType = starTypes[Math.floor(rand() * starTypes.length)];
        const starSize = 3 + rand() * 7; // 3-10 size range
        
        // Generate planets (1-8 planets)
        const planetCount = Math.floor(rand() * 8) + 1;
        const planets = [];
        
        for (let i = 0; i < planetCount; i++) {
            const planetTypes = ['Terrestrial', 'Gas Giant', 'Ice World', 'Desert World', 'Ocean World'];
            const planetType = planetTypes[Math.floor(rand() * planetTypes.length)];
            const planetSize = 0.5 + rand() * 3; // 0.5-3.5 size range
            
            // Generate moons (0-3 moons per planet)
            const moonCount = Math.floor(rand() * 4);
            const moons = [];
            
            for (let j = 0; j < moonCount; j++) {
                moons.push({
                    moon_name: `${sector}-${i + 1}-Moon-${j + 1}`,
                    moon_type: 'Rocky',
                    moon_size: 0.1 + rand() * 0.5 // 0.1-0.6 size range
                });
            }
            
            planets.push({
                planet_name: `${sector}-${i + 1}`,
        ...
        console.log('Generated fallback star system:', {
            starName: fallbackSystem.star_name,
            starType: fallbackSystem.star_type,
            planetCount: fallbackSystem.planets.length
        });
