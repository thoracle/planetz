...
export class GalacticChart {
    ...
    async fetchUniverseData() {
        try {
            ...
            if (!response.ok) {
                // Backend not available - generate fallback data (this is normal in standalone mode)
                console.log('🌌 Running in standalone mode - generating client-side universe data');
                return this.generateFallbackUniverse();
            }
            ...
            // API fetch failed (normal in standalone mode)
            console.log('🌌 API not available (standalone mode) - generating client-side universe');
            return this.generateFallbackUniverse();
        }
    }

    generateFallbackUniverse() {
        // Generate a simple fallback universe with 81 systems (9x9 grid)
        const fallbackUniverse = [];
        for (let i = 0; i < 81; i++) {
            const row = String.fromCharCode(65 + Math.floor(i / 9)); // A-I
            const col = (i % 9) + 1; // 1-9
            const starName = `${row}${col}`;
            
            // Generate random planets (1-8 per system)
            const planetCount = Math.floor(Math.random() * 8) + 1;
            const planets = [];
            for (let p = 0; p < planetCount; p++) {
                // Generate random moons (0-3 per planet)
                const moonCount = Math.floor(Math.random() * 4);
                const moons = [];
                for (let m = 0; m < moonCount; m++) {
                    moons.push({
                        moon_name: `${starName}-${p + 1}-Moon-${m + 1}`,
                        moon_type: 'Rocky',
                        moon_size: Math.floor(Math.random() * 3) + 1,
                        diplomacy: 'Neutral',
                        government: 'None',
                        economy: 'None',
                        technology: 'None'
                ...
                planets.push({
                    planet_name: `${starName}-${p + 1}`,
                    name: `${starName}-${p + 1}`, // Backwards compatibility
                    planet_type: ['Terrestrial', 'Gas Giant', 'Ice World', 'Desert World'][Math.floor(Math.random() * 4)],
                    type: ['Terrestrial', 'Gas Giant', 'Ice World', 'Desert World'][Math.floor(Math.random() * 4)], // Backwards compatibility
                    planet_size: Math.floor(Math.random() * 5) + 1,
                    size: Math.floor(Math.random() * 5) + 1, // Backwards compatibility
                    moons: moons, // Add moons array to prevent undefined error
                    diplomacy: 'Neutral',
                    government: 'None',
                    economy: 'None',
                    technology: 'None'
            ...
            fallbackUniverse.push({
                star_name: starName,
                sector: starName,
                star_type: ['G', 'K', 'M', 'A', 'F'][Math.floor(Math.random() * 5)],
                planets: planets,
                coordinates: { x: i % 9, y: Math.floor(i / 9) }
            });
        }
        
        console.log('Generated fallback universe with', fallbackUniverse.length, 'systems');
        
        // Process through chart system if available
        const ship = this.viewManager.getShip();
        const chartSystem = ship ? ship.systems.get('galactic_chart') : null;
        
        if (chartSystem && chartSystem.isOperational()) {
            this.universe = chartSystem.processUniverseData(fallbackUniverse);
        } else {
            this.universe = fallbackUniverse;
        }
